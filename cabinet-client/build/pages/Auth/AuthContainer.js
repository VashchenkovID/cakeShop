    ...
    const enterInApp = async () => {
        if (location.pathname.includes(PublicRoutesEnum.AUTH)) {
            ...
            await AuthService.registration({
                ...
                localStorage.setItem(LocalStorageKeysEnum.USER, JSON.stringify(r.data.user));
                clearInputs();
                setLoading(false);
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement("div", { className: style.Container },
            React.createElement("div", { className: style.Container__body },
                React.createElement(Text, { size: "3xl", className: style.Container__title }, location.pathname.includes(PublicRoutesEnum.AUTH)
                    ? "Регистрация"
                    : "Авторизация"),
                ...
                location.pathname.includes(PublicRoutesEnum.AUTH) && (React.createElement(TextField, { size: "s", width: "full", placeholder: "Введите имя", label: "Имя", value: name, onChange: ({ value }) => {
                        if (value) {
                            setName(value);
                        }
                        else
                ...
                location.pathname.includes(PublicRoutesEnum.AUTH) && (React.createElement(PhoneInput, { specialLabel: "Номер телефона", containerClass: style.Container__phoneContainer, inputClass: style.Container__phoneInput, placeholder: "Введите номер телефона", country: "ru", value: phone, onChange: (value) => setPhone(value
                        .split("")
                        .map((elem, index) => (index === 0 ? "7" : elem))
                        .join("")) })),
                location.pathname.includes(PublicRoutesEnum.LOGIN) ? (React.createElement(Text, { size: "s", onClick: () => {
